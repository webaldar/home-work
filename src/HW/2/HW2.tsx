...
export const HW2 = () => {
  ...
  const filterUsers = () => {
    debugger
    const filteredUsers =  users.myFriends.filter(user => user.address.city === 'Los Angeles')//'НУЖНО ПРОФИЛЬТРОВАТЬ ДРУЗЕЙ.
    console.log(filteredUsers)
    const frendUsers = {myFriends[filteredUsers] }
    // ОСТАВЛЯЕМ ТОЛЬКО ТЕХ, КОТОРЫЕ ЖИВУТ В ГОРОДЕ LOS ANGELES';
    setCurrentUsers({ myFriendsfilteredUsers });
  };
